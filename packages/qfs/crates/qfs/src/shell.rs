...
pub fn run_engine_and_reads() -> (Engine, ReadRegistry, qfs_core::SafetyMode) {
    ...
    reads = match crate::clients::live_github_client() {
        ...
        None => reads.with(
            ...
            Arc::new(crate::read_facets::ConnectAccountReadDriver::new(
                "connect a GitHub account to read it — run `qfs init`, then `qfs account add github` (GitHub reads need an authenticated token)",
            )),
        ),
    };
    reads = match crate::clients::live_slack_client() {
        ...
        None => reads.with(
            DriverId::new("slack"),
            Arc::new(crate::read_facets::ConnectAccountReadDriver::new(
                "connect a Slack workspace to read it — run `qfs init`, then `qfs account add slack`",
            )),
        ),
    };
    ...
    for (source, reason) in [
        ("mail", "connect a Google account to read mail — run `qfs init`, then `qfs account add google` (gmail reads are not available without an authenticated account)"),
        ("drive", "connect a Google account to read Drive — run `qfs init`, then `qfs account add google`"),
        ("ga", "connect a Google Analytics account to read it — run `qfs init`, then `qfs account add google`"),
