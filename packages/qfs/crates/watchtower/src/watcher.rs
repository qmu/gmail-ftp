//! **Source watchers (pollers)** (t34, RFD §8): a [`Watcher`] periodically re-runs a source query
...
/// A polling source watcher (RFD §8). Holds the source path, the poll interval (epoch seconds),
