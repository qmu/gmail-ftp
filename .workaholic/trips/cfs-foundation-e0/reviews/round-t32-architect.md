...
- **Subject**: t32 Server HTTP endpoints — new crate `cfs-http`, ADR-0004, dep_direction guard
  changes, `cfs serve` wiring, `EndpointDef.policy` column.
- **Commit**: `3779188`
...
the `cfs-http` leaf topology is the principled resolution of the CO-t30-1 / CO-t29-4
...
## Headline Question 1 — `cfs-http` leaf topology + guard changes

**Ruling: this is the principled resolution of the CO-t30-1 / CO-t29-4 collision.**

The collision I flagged earlier was: the HTTP binding must touch BOTH `cfs-server` (the
`Binding`/`ServerState`/`EndpointDef` registry, which CO-t30-1 requires to stay
runtime-free with a *synchronous* owned-snapshot `reconcile`) AND `cfs-exec` (the read
executor, whose consumer set CO-t29-4 had pinned to exactly `{cfs-cmd, cfs}`). Neither
existing crate could legally host it: putting it in `cfs-server` would drag `cfs-exec` (and
its tokio/async surface) below the spine and break the runtime-free invariant; putting it
in `cfs-cmd` would put `cfs-cmd` on `cfs-exec`/`cfs-http`, which the binary-entrypoint guard
forbids.
...
- **No spine inversion.** `crates/server/Cargo.toml` has no `cfs-exec` / `cfs-runtime` /
  `cfs-http` edge (confirmed by grep — only a comment noting it is not a runtime consumer).
  `cfs-server` stays runtime-free; `HttpBinding::reconcile` is synchronous, takes an owned
  `&ServerState`, and does the async listener work entirely in `cfs-http` (`serve.rs`).
  CO-t30-1 is intact.
- **`cfs-exec` consumer allowlist extension is a legitimate generalization, not a
  ...
  crate reaching up." `cfs-http` plays exactly the role `cfs-cmd` plays (a leaf integration
  consumer of `execute_read`), and the guard asserts (a) cfs-http depends on both
  cfs-server+cfs-exec, (b) **nothing but the `cfs` binary depends on cfs-http**, (c) cfs-http
  does NOT depend on cfs-runtime. (b)+(c) are the real guarantees; the allowlist string is
  just the surface.
- **tokio still dead-ends.** Because (b) holds, cfs-http's tokio listener is reachable only
  from the terminal `cfs` binary — the same runtime-leaf precondition t28 relied on. tokio
  is declared in `cfs-http` for the HTTP I/O domain (`net`/`io-util`/`sync`/`time`), never
  via `cfs-runtime`, and the COMMIT interpreter is untouched. No spine inversion exists.

**Scaling to t33 (ingest) / t35 (Worker):** the per-binding-leaf pattern scales cleanly. A
t33 ingest binding would be a sibling leaf (`cfs-ingest` or similar) consumed only by the
...
would admit it while the leaf guard would not catch it (the leaf guard is cfs-http-specific).
*Proposal*: at t33, generalize the leaf guard to "every cfs-exec consumer other than the
...
`CFS_HTTP_ADDR` is set to a non-loopback bind. *Proposal*: add a per-connection read deadline
...
2. **Query eval reuse — confirmed no new eval logic.** `dispatch_inner` calls
   `cfs_exec::build_plan` + `cfs_exec::execute_read` (`handler.rs:130,136`); the tests use an
...
4. **Error mapping / sanitization — confirmed, with one note.** `HttpError` maps
   ...
   `eval` body forwards `e.message` verbatim — this relies on the cfs-exec contract that
...
6. **The two bug fixes — confirmed sound, not masking.**
   ...
   - *Non-fatal listener bind failure* (`serve.rs:77-101`): a bind error logs a warning and
     ...
     core of `cfs serve` still runs. *Observation*: an operator who *expects* HTTP serving and
     silently gets config-only-on-port-conflict might be surprised. *Proposal*: consider a
     `CFS_HTTP_REQUIRE_BIND=1` opt-in (a later ticket) that makes bind failure fatal for
...
| 6 | Silent config-only fallback on bind conflict | Low | Optional `CFS_HTTP_REQUIRE_BIND` later. |
