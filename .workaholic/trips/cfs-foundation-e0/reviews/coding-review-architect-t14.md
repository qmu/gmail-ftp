...
`lower.rs::lower_predicate` lowers the parser `Expr` straight into `cfs_types::Predicate`
...
dependency edge is on `cfs-parser`, not on the evaluator's schema-threading IR. `cfs_core::plan`
...
- Edges are acyclic and match `ARCHITECTURE.md`: `cfs-pushdown → {cfs-driver, cfs-types,
  cfs-plan, cfs-parser}` (Domain, no I/O/async/vendor), `cfs-engine → {cfs-pushdown, cfs-types}`,
  `cfs-core → cfs-pushdown` (integration seam). No back-edges; both sit below `cfs-core`.
- `cfs-engine` is genuinely serde-only: its `Cargo.toml` declares only `cfs-pushdown` +
  `cfs-types`, and the ADR's `cargo tree` closure is the serde family + `thiserror`. No DuckDB,
...
through `Schema::expand` (already implemented in `cfs-types`) and the aggregate output types, so
...
dependency-spine block were updated to match (Domain `cfs-pushdown`, Infrastructure `cfs-engine`,
`cfs-core → cfs-pushdown` seam). The structured-error discipline (`LowerError`/`PlanError`/
