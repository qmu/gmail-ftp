...
- Method: throwaway external-consumer crate (`/tmp/t18-e2e`, own `[workspace]`, path-deps on
  ...
  `http_get_node`) through `cfs_runtime::Interpreter::commit` (sync→async `PlanApplierBridge`),
  auth from a `cfs_secrets::InMemoryStore`, responses decoded by `cfs_codec::JsonCodec`.
...
- (a) **Auth injected**: the loopback server received `Authorization: Bearer <token>` on the
  wire — the secret resolved from the `cfs_secrets` store and was injected into the request.
...
driver-http apply path. This protects the stakeholder-visible promise ("cfs never logs your
