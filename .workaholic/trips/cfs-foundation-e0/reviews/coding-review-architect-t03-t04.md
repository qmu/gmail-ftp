...
dependency spine §4) and the RFD §3 frozen list (`.workaholic/RFDs/0001-cfs-architecture.md`).
...
lexer living in `cfs-lang` rather than `cfs-parser`: it keeps the closed core dependency-free
...
**serde placement — CORRECT per model.** `serde` is a dependency of `cfs-parser` only;
`cfs-lang/Cargo.toml` stays empty. The AST supplies its own `serialize_span` projection rather
than deriving `Serialize` on `cfs_lang::Span`, precisely so the zero-dep closed-core crate is
...
`crates/core/Cargo.toml`. `cfs-parser` depends on `cfs-lang` + `winnow` + `serde` only; it
does NOT depend on `cfs-core` (the reserved `cfs-core -> cfs-parser` edge is declared-but-not-
wired in core's Cargo.toml with the C5 note, correctly one-directional). `cfs-cmd` still
depends on `cfs-core` + `cfs-server` only (G5 intact — the `cfs-lang`/`cfs-parser` lines in
...
`cfs-lang` and flow through both crates without duplication.
