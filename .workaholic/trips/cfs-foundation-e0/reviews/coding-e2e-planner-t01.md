...
Target: `cfs` CLI scaffold (ticket t01)
Date: 2026-06-22
Binary under test: `target/debug/cfs` (`cfs 0.0.0`), built via `cargo build -p cfs`
...
- `cfs run --stmt/-e <STMT>` — statement is a **required option**, no positional form.
- `cfs serve <CONFIG>` — config is a **positional argument**.
- `cfs` (no subcommand) — interactive FTP-like shell (currently stubbed).
- Global `--json` accepted both before the subcommand (`cfs --json run …`) and after
  (`cfs run … --json`); both produce a byte-identical JSON envelope.
...
|---|----------|---------|------|--------|
| 1a | version | `cfs --version` | 0 | **PASS** — prints `cfs 0.0.0` |
| 1b | version short | `cfs -V` | 0 | **PASS** — prints `cfs 0.0.0` |
| 1c | help | `cfs --help` | 0 | **PASS** — usage lists `run`, `serve`, `help`; global `--json`; documents no-subcommand shell. Coherent. |
| 2 | run pipe expr | `cfs run -e 'FROM /mail \|> WHERE x \|> SELECT y'` | 1 | **PASS** — `error[not_implemented]: not yet implemented: run` on stderr; empty stdout; no panic. |
| 3 | run positional | `cfs run 'FROM /mail'` | 2 | **PASS (with observation)** — positional form is **not a supported form**; `run` requires `--stmt`/`-e`. Result is a clean clap usage error (`error: unexpected argument 'FROM /mail' found` + usage), exit 2, no panic. Correct rejection, but see Concern 1. |
| 4 | json error envelope | `cfs --json run -e 'FROM /mail'` | 1 | **PASS** — valid JSON envelope on stderr; **confirmed parseable by python3 AND jq**; exit 1. Subcommand-position `--json` byte-identical. |
| 5 | serve nonexistent | `cfs serve /tmp/nonexistent.cfs` | 1 | **PASS** — `error[not_implemented]: not yet implemented: serve`, exit 1, no panic. `--json` form also valid (`code: not_implemented`, `message: not yet implemented: serve`). Note: stub short-circuits before any file-existence check, which is correct for a not-implemented surface. |
| 6 | no-args shell stub | `printf '' \| cfs`, `cfs < /dev/null`, `cfs --json < /dev/null` | 1 | **PASS** — does **not** hang/block on stdin; stubs cleanly with `error[not_implemented]: not yet implemented: shell` (JSON: `code: not_implemented`, `message: not yet implemented: shell`), exit 1, no panic. Guarded with a 10s timeout; returned immediately. |
| 7 | bogus subcommand | `cfs bogus` | 2 | **PASS** — clap usage error `error: unrecognized subcommand 'bogus'` + usage hint, exit 2, no panic. |
...
(aliasing `--stmt`) in a later language-core ticket, so `cfs run 'FROM /mail'` works. Pure
