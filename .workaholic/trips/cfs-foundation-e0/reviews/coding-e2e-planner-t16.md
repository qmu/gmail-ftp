...
Target: t16 — `cfs_driver_local::LocalFsDriver` + runtime bridge end-to-end
Date: 2026-06-23
...
ledger:
  ...
  #1 UPSERT -> applied
  #2 REMOVE -> applied
final fs state: orig.txt exists=false  copy.txt exists=true  copy content="hello-cfs-e2e"
```
...
- **Proposal:** preserve the structured `LocalError` discriminant across the
  ...
  `CapabilityDenied` to dedicated `EffectError` variants (or carry a stable `cfs.*` code)
