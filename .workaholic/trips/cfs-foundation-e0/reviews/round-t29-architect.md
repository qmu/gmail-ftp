...
- **Status**: under-review
- **Reviewing**: Constructor commit `1c52771` — ticket t29 (CLI one-shot + the SELECT read-path executor), new crate `crates/exec/`, plus `crates/cmd/` and the `cfs` bin.
...
The t20 carry-over is **genuinely closed**: `cfs-exec` implements the full
parse→resolve→plan(`cfs-core::plan_query`/`cfs-pushdown`)→driver scan(`ReadDriver`)→`cfs-engine`
...
genuine gap (no mechanical dep-guard on `cfs-exec` itself) and three carry-overs.
...
`cfs-core`, `cfs-runtime`, and `cfs-exec` registries) and **out of t29's scope**. Recorded as
**CO-t29-1**.

**(c) Does keeping `cfs-exec` runtime-free hold long-term, or will real-driver COMMIT force a
`cfs-exec → cfs-runtime` dependency and collapse the separation?** It holds, and the separation
...
`cfs_core::RecordingApplier` test double, and the doc comment defers real COMMIT to "the runtime
interpreter (t30+)". The key structural fact: the `cfs` **binary** is already an allowed
`cfs-runtime` consumer (`dep_direction.rs` line 335). So the real-driver COMMIT wiring can live in
the binary / a bootstrap leaf that owns both `cfs-runtime` and `cfs-exec` — `cfs-exec` need not gain
the runtime edge itself. If, instead, a future ticket makes `cfs-exec` depend on `cfs-runtime`
directly, the generic leaf-confinement check (`dep_direction.rs` lines 288–316) fires the moment
`cfs-cmd → cfs-exec` is present, because `cfs-exec` would then be a non-leaf runtime consumer
(`cfs-cmd` depends back onto it). **That guard is the structural safety-net that prevents the
collapse from happening silently.** So the separation is enforceable, not merely aspirational —
but see Observation 1: it is enforced for `cfs-exec` only *transitively/negatively*, never
*positively*.
...
- **Runtime minimal spine untouched**: `cfs-exec` does **not** depend on `cfs-runtime`
  (`crates/exec/Cargo.toml` deps = core/parser/pushdown/engine/tokio/async-trait/serde). The
  runtime confinement test (`dep_direction.rs:199–346`) fires only on `cfs-runtime` consumers, so
  it is unaffected.
- **`cfs-plan` purity closure stays tokio-free**: the purity test (`crates/plan/tests/purity_deps.rs`)
  BFSes only `cfs-plan`'s transitive closure. Nothing in the pure spine depends back onto
  `cfs-exec`, so tokio in `cfs-exec` cannot enter that closure. Confirmed structurally.
- **`cfs-cmd` stays logic-free (C4)**: the new `cfs-cmd → cfs-exec` edge is permitted — `cfs-exec`
  is none of the five forbidden domain crates (`cfs-lang/plan/driver/codec/parser`). The C4 test
  (`dep_direction.rs:61–93`) only forbids those five + asserts core/server presence; it does not
  reject `cfs-exec`. `dispatch_run` (`crates/cmd/src/lib.rs:209–245`) is genuinely thin: it resolves
  the statement source, picks the format by TTY, builds an empty `ReadRegistry`, and delegates all
  execution to `cfs_exec::run_oneshot`. No domain logic leaked into cmd.
- **No spine inversion**: no spine crate depends on `cfs-exec`; the edge flows up only
...
One nuance for the record: `cfs-cmd::report_error` (the *non-`run`* arms: shell/serve/account) still
...
and the second tokenizer is deleted. That touches `cfs-parser` grammar and is **out of t29 scope**.
...
### Observation 1 (genuine gap, non-blocking) — `cfs-exec`'s purity is enforced only negatively; add a positive mechanical guard

The whole topology rests on the claim "`cfs-exec` sits above the spine and the pure spine never
depends back onto it." Today that holds, but **no test asserts it**. The runtime confinement test
pins `cfs-runtime`'s allowlist and `nothing_depends_on_cmd` pins `cfs-cmd`'s consumers; there is no
analogous guard for `cfs-exec`. The only thing that would catch a regression (e.g. a future
`cfs-core → cfs-exec` edge, or `cfs-exec → cfs-runtime`) is the *generic* runtime-leaf check, and
only once such an edge actually pulls the runtime in. A spine crate gaining a plain
`→ cfs-exec` edge (no runtime involved) — which would invert the layering and is exactly the
structural property the crate docs promise — would pass CI silently.

**Proposal (structural, small):** add a `cfs-exec` confinement test to `crates/cmd/tests/dep_direction.rs`
(or a sibling), asserting two directions, mirroring the existing runtime/secrets/http-core guards:
(a) `cfs-exec`'s workspace deps are exactly `{cfs-core, cfs-parser, cfs-pushdown, cfs-engine}` (and
NOT `cfs-runtime`), and (b) the only crate depending on `cfs-exec` is `cfs-cmd`. This converts the
...
- **CO-t29-1 — Unified driver-facet registration.** A real driver registers three disjoint facets
  ...
  registration is impossible. Cross-cutting (`cfs-core` + `cfs-runtime` + `cfs-exec`). This is the
  real answer to the headline question's part (b).
- **CO-t29-2 — Single error-envelope builder.** `cfs-exec::output::error_envelope` (superset) and
  `cfs-cmd::report_error` (legacy two-field) are two envelope builders. Once the shell/account arms
...
- **CO-t29-3 — Lexer-level absolute-path bit.** Preserve absolute-vs-relative on the path token in
  `cfs-parser` so the addressing gate runs over the AST and the ad-hoc `addressing::lex_words`
  second tokenizer is deleted. Safe to defer (no driver mis-resolution hazard today, see surface 5).
- **CO-t29-4 (optional) — `cfs-exec` positive dep guard.** See Observation 1; promote if not folded
  into a t29 fix.
