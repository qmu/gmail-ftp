...
- **Method**: a throwaway external-consumer crate (`/tmp/t21-e2e-planner`, own `[workspace]`,
  ...
  `cfs_runtime::{Interpreter, DriverRegistry, PlanApplierBridge}`. `MockDriveClient` was seeded
...
`cfs_secrets::Secret` and pushed through every channel a leak could ride. Captured proof:
