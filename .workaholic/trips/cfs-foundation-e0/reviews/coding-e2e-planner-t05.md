...
- Target: t05 — Type & schema model (`cfs-types`)
- Method: external consumer. A throwaway crate in `/tmp/t05-e2e` (own `[workspace]`,
  path-dep on `crates/types`, plus a compile-only check against the `cfs-core`
...
`cfs_core` re-export (compile-only) — the path real consumers (CLI `-json`, evaluator,
drivers) will use.
...
is reachable through `cfs_core`. **E2E approved.**
