...
- Target: t27 — `cfs-secrets` crate + Engine `with_secrets` + `cfs account` stubs
...
0600 + atomic write) is correct. The spine is a clean leaf (`cfs-secrets → cfs-types`) locked by
...
4. **No backend formats/logs plaintext.** Every `SecretError` variant is secret-free by
   ...
   `cfs-cmd` logs only the `feature` label via `tracing`, never a credential.
...
(or all, for the admin `cfs account list` surface). `resolve()` filters `available` to the passed
...
`cfs-secrets` is a clean leaf: `Cargo.toml` declares exactly one workspace dep, `cfs-types`
...
cfs-types among workspace crates and (b) cfs-core consumes secrets. The Engine threads
...
> Minor (S3, doc fidelity): **`ARCHITECTURE.md` was not updated for `cfs-secrets`.** The Crate map
> table (line 13+) and the Dependency-spine block (line 30+) list every other crate but omit
> `crates/secrets` / `cfs-secrets` and the `cfs-secrets → cfs-types` + `cfs-core → cfs-secrets`
...
## 6. E4 consumability + `cfs account` park honesty — PASS
...
The `cfs account` park is **honest**: `cmd/lib.rs` declares the full structured `AccountVerb`
enum (`add|list|use|remove` with typed args), parses cleanly (tests assert `cfs account list/add`
