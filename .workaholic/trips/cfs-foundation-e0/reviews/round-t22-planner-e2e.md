...
```
source ~/.cargo/env
cargo test  -p cfs-driver-objstore --test planner_e2e     # 16 external E2E scenarios — all pass
cargo test  -p cfs-driver-objstore                          # full suite: 45 unit + 16 E2E — all pass
cargo test  -p cfs-driver-objstore --lib sigv4              # 5 offline SigV4 tests incl. the AWS vector
cargo clippy -p cfs-driver-objstore --tests -- -D warnings  # clean
```
...
`cargo build -p cfs-driver-objstore --target wasm32-unknown-unknown` does NOT build standalone: `cfs-driver-objstore → cfs-runtime → tokio (rt-multi-thread)` triggers `tokio`'s `compile_error!("Only features sync,macros,io-util,rt,time are supported on wasm.")`. This is NOT a t22 regression and NOT a behavioral defect I can exercise:
- It is a **shared workspace constraint** identical to the already-accepted t23 `cfs-driver-cf` (both are runtime leaves that bridge through `cfs-runtime`).
