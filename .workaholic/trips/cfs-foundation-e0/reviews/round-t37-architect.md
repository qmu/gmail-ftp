...
(d) **Serve fire paths failing closed by default is ACCEPTABLE for E0.** Both `cfs-cron` and
`cfs-watchtower` committers call `require_ack(&plan, RunMode::Server, Ack::Absent)` after the t35
...
**Placement/confinement (correct).** `IrreversibleGuard`/`RunMode` in `cfs-core` (the shared
commit-seam hub both faces route through — pure data-in/out, wasm-clean, no I/O); `LogScrubber` in
`cfs-cmd` (the binary's logging init, the right home for a sink wrapper); the gate in
`cfs-secrets/tests` (a mechanical CI assertion). No spine inversion: the guard is pure over
`cfs-plan::Plan`, nothing in the pure spine depends back on the consumers. Confinement holds.
