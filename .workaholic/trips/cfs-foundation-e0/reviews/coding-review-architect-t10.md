...
- `cfs-runtime` depends on `cfs-plan` + `cfs-types` only (`Cargo.toml`), plus tokio/futures/
  async-trait/serde/tracing. **No `cfs-core` dependency** — the runtime → core inversion the
  t07 guidance warned against is avoided, and the runtime walks `cfs-plan` effect types
  exclusively.
- **tokio is genuinely confined.** `grep` confirms no `crates/*/Cargo.toml` other than
  `crates/runtime` references `cfs-runtime`, i.e. **nothing depends back up onto the runtime**.
  `cfs-plan`'s `purity_deps.rs` test walks `cfs-plan`'s *forward* resolve closure only; since
  the edge is `cfs-runtime → cfs-plan` (never the reverse), tokio can never enter cfs-plan's
...
`assemble` rebuilds the ledger by walking `cfs_plan::topo_order` (stable Kahn with NodeId-sorted
...
- **E4 sync→async adapter gap:** keeping the async `ApplyDriver` in `cfs-runtime` (rather than in
  `cfs-driver`) is the right call — it is what lets `cfs-plan`/`cfs-driver` stay I/O-free while
...
- **O3 — The acyclic/confinement guarantee is not yet mechanically asserted for `cfs-runtime`.**
  `dep_direction.rs` has no `cfs-runtime` case, so "nothing depends back up onto the runtime"
  and "tokio stays out of the spine crates" rest on review, not CI. *Proposal:* add a
  `dep_direction` assertion that no spine crate (`cfs-core`/`-plan`/`-driver`/`-types`/`-codec`/
  `-lang`/`-parser`) has `cfs-runtime` (or tokio) in its resolved closure — the mechanical
