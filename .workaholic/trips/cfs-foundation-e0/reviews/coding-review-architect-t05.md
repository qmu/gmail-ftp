...
The new `cfs-types` leaf is a faithful, well-bounded realization of RFD §4/§5 and the
...
- **`cfs-types` is a true leaf.** `crates/types/Cargo.toml` declares only `serde` +
  `serde_json` (both workspace-pinned); zero `cfs-*` path deps. The two leaf-preserving
  moves are correct and consistent:
  - `DriverId` is defined *inside* `cfs-types` (`schema.rs:18`) as an owned newtype, not
    imported from `cfs-driver`. This is the right call for leaf status (see D2 verdict).
  - `SchemaSource::describe` takes `&[Name]` (a logical segment list) rather than the
    driver `Path`, so the trait surface does not pull `cfs-driver` upward into the leaf.
    The doc comment (`schema.rs:298-311`) names the boundary adaptation explicitly.
- **New edges are acyclic.** `cfs-codec → cfs-types` and `cfs-core → cfs-types` both
  point at a leaf, so no cycle is possible. `cfs-codec → cfs-driver` (D1) is untouched and
...
- **The dep-direction test locks it.** `dep_direction.rs::types_is_a_leaf_and_codec_depends_on_it`
  asserts (a) `cfs-types` depends on no workspace crate, (b) `cfs-codec` depends on it,
  (c) `cfs-core` depends on it. The binary-deps test was updated to include `cfs-types`
...
hardcoded list. When E1+ adds crates (`cfs-lang` schema use, future `cfs-eval`), the
list must be kept in sync or a new crate could depend *on* `cfs-types`'s position
...
- The typed-predicate IR is deliberately the type model's *own* IR, distinct from the
  ...
  the parser vendor-free and makes `cfs-types` the single home of typing rules. The
...
| | type | columns | extras |
|---|---|---|---|
| `cfs_driver::NodeSchema` | `driver/src/lib.rs:81` | `Vec<String>` (untyped names) | `archetype: Archetype` |
| `cfs_types::Schema` | `types/src/schema.rs:149` | `Vec<Column>` (typed, nested) | — |
...
1. `NodeSchema` *absorbs* `cfs_types::Schema` (its `columns: Vec<String>` becomes
...
with `cfs_types::Schema`; decide archetype placement" so the deferral is owned.

**D2 verdict — `DriverId`-in-types is the right call.** The alternative (a driver-owned
identity imported into types) would force `cfs-types → cfs-driver`, destroying leaf
...
spine (a driver-side key can be a newtype over, or `From`-convert to, `cfs_types::DriverId`),
so D2 is sound. Worth one carry-over note that the driver mount key should reuse
`cfs_types::DriverId` rather than mint a parallel one.

## 3. Governance / G6 — PASS, one boundary choice to ratify

- **No vendor type leaks.** Every public type is owned (`String`, `Vec`, `i64`, etc.) or
  built from owned types. No driver SDK type, no `cfs-driver`/vendor handle in any public
...
- **`serde_json::Value` as the `Json` carrier — acceptable, but it IS a third-party type
  ...
  `Value` is pragmatic and keeps `cfs-types` "serde-family only", and the Cargo.toml
  comment frames it as "a pure data tree, not I/O." I accept it, with the observation that
  it is technically a vendor (crate) type in `cfs-types`'s public API — the one place the
  ...
  **Proposal (defer):** if strict ownership is later desired, introduce an owned
  `cfs_types::Json` tree (a thin sum type) and convert at the codec boundary; for now the
...
`Schema`/`Value`/`TypeError` surface, matching the doc's "consumers depend on cfs-core
...
1. **Two-schema reconciliation (E1/E4):** record an explicit carry-over to reconcile
   `cfs_driver::NodeSchema.columns: Vec<String>` with `cfs_types::Schema`, and decide
...
2. **`DriverId` reuse (E4):** the driver mount/registry key should reuse
   `cfs_types::DriverId`, not mint a parallel identity.
