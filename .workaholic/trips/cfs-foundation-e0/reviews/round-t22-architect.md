...
Design reference: `.workaholic/trips/cfs-foundation-e0/designs/design-vt22.md`
...
Solid. `secret_access_key` and `session_token` are `cfs_secrets::Secret`; the secret is `expose_str`'d **only** inside `derive_and_sign`/`sign` to compute the signing key and the redacted `Authorization` value. No DTO, no `ObjError` arm, and no `RecordedCall` carries a credential (every error arm is a path / verb-label / `&'static str` op / status / fixed reason). The planted-canary test drives all seven `ObjError` arms through `Debug` and `Display` and asserts neither the full secret nor the `deadbeef` fragment appears; `signed_request_debug_redacts_the_authorization` asserts the signed request's `Debug` contains `cfs_secrets::REDACTED` and not the secret. The structural guarantee (secrets confined to the signer, redacted on every observable surface) holds. **Approve.**
...
Confirmed. `cfs-driver-objstore` is appended to `runtime_consumers_allowed` in `crates/cmd/tests/dep_direction.rs` (the one-line reviewable signal). The crate bridges its synchronous `PlanApplier`/`SharedApplier` to the async `ApplyDriver` via `PlanApplierBridge` and nothing depends back onto it (tokio dead-ends at the leaf), so the spine is not inverted. It rides the local `HttpExchange` seam over `cfs-http-core` rather than depending on `cfs-driver-http` — preserving the independent-leaf property and matching the established `cfs-google-auth`/`cfs-driver-cf` precedent. The `worker` crate is correctly a target-cfg dependency. **Approve.**
