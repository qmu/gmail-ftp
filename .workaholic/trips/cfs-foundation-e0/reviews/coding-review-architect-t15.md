...
codecs funnel through one `serde_json::Value`↔`cfs-types` bridge, the vendor parsers are genuinely
...
1. **G6 / boundary — no vendor leak (confirmed mechanically).** `mod convert;` and `mod nested;` are
   **private** (lib.rs:19-21); `convert` is *not* re-exported. The five `pub fn`s in `convert.rs` that
   ...
   `cfs_types`/`CfsError` re-exports — carries **only owned `cfs-types` and `CfsError`** types. No
...
3. **Registry fit (RFD §3).** `CodecRegistry` (registry.rs:154) is the third registry, identical in
   ...
   `with_builtins` loads the canonical `cfs_codec::builtin_codecs()` set (single source of truth in the
...
acyclic spine is preserved — `cfs-codec → cfs-driver` (shared error) and `cfs-codec → cfs-types` are the two
