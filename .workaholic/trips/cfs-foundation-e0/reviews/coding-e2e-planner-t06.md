...
- Method: external-consumer E2E. A throwaway crate OUTSIDE the cfs workspace
  (`/tmp/cfs_e2e_t06`, own `[workspace]`, path-deps on `crates/{core,parser,driver,types}`,
  no production code) implements `cfs_core::Driver` out-of-crate, registers fakes in a
  `MountRegistry`, parses real source with `cfs_parser::parse_statement`, and drives
  `cfs_core::Resolver::resolve_statement`, branching on every `ResolveError` arm.
...
- **O3 — Strength: the public surface is genuinely external-implementable.** The fake drivers
  were written with zero access to any `cfs-*` internal item — only re-exported public types
