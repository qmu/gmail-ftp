...
`cfs-parser`'s public surface; the ADR is evidence-driven rather than a restatement of the
RFD §9 default; the spike is correctly quarantined; and the skeleton's AST/`Stmt` shape is
a faithful, growable subset of the frozen `cfs-lang` vocabulary. The minor suggestions
...
- The entire public re-export list is owned: `ast::{CmpOp, Expr, Literal, Path, PipeOp,
  Stmt}`, `error::{ParseError, ParseErrorCode}`, and the `cfs_lang` re-exports. I traced
...
- `parse_statement(&str) -> Result<Stmt, ParseError>` is the stable signature the ticket
  required (acceptance criterion C5 direction `cfs-core → cfs-parser`, no cycle).
...
which has the full `cfs_lang::KEYWORDS` set available, should classify by membership/case
...
and every public type are untouched, so `cfs-core` and all E1+ callers are insulated. I
...
that the t01 CI matrix actually contains a `cargo build -p cfs-parser --target
...
**Verdict: PASS — winnow/chumsky stay out of the production graph except `cfs-parser → winnow`.**

- `spikes/parser-spike/Cargo.toml` has `publish = false` and is the **only** crate depending
  on `chumsky`. `cfs-parser/Cargo.toml` depends on `winnow` (and `cfs-lang`) and **not** on
  `parser-spike` — I confirmed there is no `parser-spike` path dependency anywhere in
  `crates/`. So the production dep graph is exactly `cfs-parser → winnow` (+ `cfs-lang`);
...
- Loser banner: PASS. `chumsky_spike.rs` carries an explicit "THROWAWAY SPIKE — NOT
  ...
  and `lib.rs` header both repeat "THROWAWAY / NOT production / `cfs-parser` does NOT depend
...
(`cfs` / the server) depends on `cfs-core`-side crates, not on `parser-spike`, so the spike
...
- **Frozen-vocabulary alignment: PASS with one nuance.** `grammar.rs` sources `FROM`,
  `WHERE`, `SELECT` from `cfs_lang::Keyword::{From,Where,Select}.text()` (the frozen RFD §3
  enum), so the closed core has one home (boundary B6). The AST `CmpOp` subset (`Eq, Ne, Lt,
  Gt, Le, Ge, Like`) is a strict subset of `cfs_lang::OPERATORS`, and the keyword golden/
  freeze test in `keywords.rs` locks the 38-keyword / 15-operator counts — so the parser
  cannot invent a keyword outside §3 without the freeze test catching the `cfs-lang` change.
- The ADR/A4 claim that "keyword surface text is sourced from `cfs_lang::Keyword`, not
...
are hand-typed string literals rather than sourced from `cfs-lang`:
...
`AND`, `LIKE`, and `|>` are **operators** (they live in `cfs_lang::OPERATORS`, not in the
...
are duplicated between `cfs_lang::OPERATORS` and `grammar.rs`, and could drift. For the E0
subset this is harmless (operators are frozen and the set is tiny). *Proposal:* in E1, give
`cfs-lang` a typed operator accessor (an `Operator` enum mirroring `Keyword`, or named consts
like `cfs_lang::op::AND`) and have the grammar reference those, so operators get the same
...
statement-level AST is "slated to live in `cfs-lang` in E1" and that E0 keeps a local copy.
That is a reasonable E0 decision, but it means E1 faces a *relocation* (move `Stmt`/`PipeOp`/
`Expr` from `cfs-parser` to `cfs-lang`) that **will** touch the `pub use` surface of
`cfs-parser` — i.e. the "no public signature changes on a library swap" reversibility
...
4. (2b) Testing roles: confirm t01 CI actually runs the `wasm32` `cfs-parser` build the ADR cites.
...
6. (4a) E1: give `cfs-lang` a typed operator surface (`AND`/`LIKE`/`|>`) so operators get the
   same single-home guarantee keywords have (the one real, minor fidelity gap).
7. (4b) Informational: plan E1's AST relocation to `cfs-lang` as an explicit reviewed
...
results (cfs-parser 7 tests, spike 3 tests green) and E2E/CLI validation are the Constructor's
