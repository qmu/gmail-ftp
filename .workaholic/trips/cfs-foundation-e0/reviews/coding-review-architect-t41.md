...
- Status: under-review
- Target: t41 `cfs-driver-ga` (commit `40f5b7d`)
...
"turn a raw GA 400 into a structured cfs error" goal the module docs claim.
...
- **Capability gate**: `GaDriver::caps_for` returns `Capabilities::from_verbs(&[Verb::Select])`
  ...
  No write verb is ever present, so `cfs_driver::check_capability` rejects
...
`cfs-driver-ga` holds no token. `GoogleApiGaClient` wraps `Arc<GoogleApiClient>` (t19), which
...
`reqwest` confinement: `Cargo.toml` depends on `cfs-google-auth` + `cfs-http-core` only for
HTTP — **no `reqwest`, no `cfs-driver-http`**. ga rides the runtime-free `HttpExchange` seam
over the shared `cfs-http-core` DTOs, so reqwest stays in `cfs-driver-http` and ga stays a
runtime leaf. Verdict: **confined**.

## 5. Spine — SOUND

`cfs-driver-ga` depends on `cfs-runtime` (for `PlanApplierBridge`), so the generic leaf-
confinement check (`dep_direction.rs` test (b)) applies: nothing depends back onto ga, so tokio
dead-ends in this leaf. The named allowlist append (`"cfs-driver-ga"` at line 328) is the
