...
The split is `default = ["runtime"]`, `runtime = ["dep:cfs-runtime"]`, with the optional
`cfs-runtime` dependency the *only* thing the feature gates. What makes this clean rather
than `#cfg`-rot:
...
- **No runtime type leaks into the pure subset.** `cfs_runtime::{SharedApplier, EffectError,
  ...
  references only `cfs_plan` types — so the introspective + apply core is genuinely closed
  over pure leaves (`cfs-driver`, `cfs-plan`, `cfs-types`, `cfs-secrets`, `cfs-http-core`,
  serde). None of those pull tokio.
- **The closure is real, not asserted.** `cfs-http-core` is independently pinned as a pure
  leaf carrying no reqwest/tokio (`dep_direction.rs:422`), and the Slack transport is a
  *local* `HttpTransport` trait (`client.rs:52`) rather than a dep on `cfs-driver-http`, so
...
transplants with no per-driver cleverness: gate `dep:cfs-runtime` + the `runtime_bridge`
...
**Ruling: reimplementing was correct for t25; a shared `cfs-crypto-core` leaf is worth a
...
runtime-bearing `cfs-driver-objstore` — a leaf-confinement violation (and one the
`dep_direction.rs` generic leaf check would not even catch, since it guards `→ cfs-runtime`
...
the drift hazard the team already paid down once — the t19 `cfs-http-core` extraction exists
...
`cfs-crypto-core` **pure leaf** — `sha256`, `hmac_sha256`, `hex_lower`, `constant_time_eq` —
structurally a twin of `cfs-http-core` (no reqwest/tokio/runtime, depends on at most
`cfs-types` or nothing), and re-point both objstore and slack at it. This:
...
`cfs_http_core::HttpRequest` whose redacting `Debug` hides it, and dropped. `SlackError` is
...
**Confirmed.** The Slack crate is a clean runtime leaf: it depends on `cfs-runtime`
*optionally* and nothing depends back onto it. The allowlist append is present —
`"cfs-driver-slack"` is in `runtime_consumers_allowed` (`dep_direction.rs:333`) — and the
generic leaf check (`b`) plus the named-identity check (`b'`) both cover it without further
edit. The driver deliberately does **not** dep on `cfs-driver-http`, instead trading in the
shared `cfs-http-core` DTOs through a local `HttpTransport` seam (`client.rs:52`), which keeps
...
tokio-exclusion test, and (b) file the `cfs-crypto-core` shared-leaf extraction as a
