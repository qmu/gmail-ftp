...
- Method: throwaway external consumer crate in `/tmp/t15-e2e` (own `[workspace]`,
  ...
  `cfs_core::CodecRegistry`, `cfs_codec::{Codec, builtin_codecs, expand, access, access_row}`,
  `cfs_types::{Value, Row, RowBatch, Schema, ColumnType}`.
...
the cfs value proposition ("a markdown/JSON blob becomes a *queryable, editable*
