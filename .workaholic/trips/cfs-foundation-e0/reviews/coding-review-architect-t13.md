...
(`cfs-driver → cfs-types`, existing `cfs-driver → cfs-plan`) are acyclic and locked by
...
2. **Purity guard (G3)** — the introspective half returns owned DTOs or `cfs_types::Schema` by
...
4. **Carry-overs**
   - *NodeSchema absorption*: correct. `NodeDesc { archetype, schema: cfs_types::Schema }` means
     `DESCRIBE` and type-checking (t05) share one schema; an adapter would have created a second
     schema type to keep in sync forever. The new `cfs-driver → cfs-types` edge is acyclic because
     `cfs-types` is a true leaf (asserted in `dep_direction.rs`).
   - *Path↔VfsPath adapter*: genuinely lossless (`from_vfs(p.to_vfs()) == p`, byte-for-byte,
     ...
     types in two crates is the correct way to keep the spine acyclic (`cfs-plan` cannot name
...
5. **Spine / G6** — `dep_direction.rs` now asserts both `cfs-driver → cfs-types` and
   `cfs-driver → cfs-plan` and re-asserts `cfs-types` leaf-ness, so the new edges are mechanically
...
via **longest-mount-prefix** matching. What shipped is `Driver::mount() -> &str` and a `cfs-core`
...
- The `Driver` trait exposes **no** `DriverId` accessor, yet `Target`/`EffectNode` are keyed on
  `cfs_types::DriverId`. Something must map a resolved driver → its `DriverId` to build effect
...
decision (CfsError/Path in `cfs-driver`, re-exported by `cfs-core`) is consistently reflected in
ARCHITECTURE.md, the error module doc, and the re-export list. `cfs-core::lib` re-exports the full
contract surface so consumers see one `cfs_core::*` face — translation fidelity between the layered
