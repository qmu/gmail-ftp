...
- **A2 — Crate granularity matches the ticket's eight-crate split.** I do not propose a
  coarser or finer split; the ticket's `cfs / cfs-cmd / cfs-core / cfs-lang / cfs-plan /
  cfs-driver / cfs-codec / cfs-server` (+ `cfs-parser` from t02) is the contract. My job
...
- **A3 — wasm32 is a constraint, not a deliverable.** `wasm32-unknown-unknown` CI wiring
  ...
  threads / `std::fs` / sockets in `cfs-core/cfs-lang/cfs-plan/cfs-driver/cfs-codec`).
...
|---|---|---|---|
| One binary, CLI **and** server | §1, §9 | `cfs` (bin) → `cfs-cmd` | Single `[[bin]]`; `main.rs` is thin and delegates to `cfs-cmd::run(argv)`. CLI and `serve` are two dispatch arms of the *same* binary, not two programs. |
| Pipe-SQL language, **frozen** keyword set | §2.2, §3 | `cfs-lang` | The reserved-keyword `const` set lives in exactly one place. AST sum types (E1) land here. Frozen-ness is structurally enforceable because there is one home. |
| Effect-plan (effects-as-data, typed DAG) | §2.3, §6 | `cfs-plan` | `enum Effect`, `struct Plan`, `irreversible` flag. The purity invariant (`fn … -> Plan`) is anchored by *the existence of this crate as the only effect type*. |
| Three open registries (paths / procs+fns / codecs) | §3 | `cfs-core` (`MountRegistry`, `ProcRegistry`, `CodecRegistry`) | The registries are the *governance mechanism* — "new backend = zero keywords". They must sit in the shared engine glue so both CLI and server resolve through the same surface. |
| Driver contract (namespace, capabilities, procedures, pushdown, prelude) | §5 | `cfs-driver` (`trait Driver`, `Archetype`, `Capabilities`, `ProcedureDecl`, `AliasFn`) | The consumer-side narrow trait + owned-DTO rule. This is the seam every E4 driver fills. |
| Codecs (pure `bytes ↔ rows`) | §4, §3 | `cfs-codec` (`trait Codec`) | A separate registry/trait so codecs compose over *any* blob source independent of driver identity. |
| Server-is-a-driver (`/server/...`, bindings) | §8 | `cfs-server` (`serve(config)` stub, `/server` mount) | The server is structurally a `Driver` over `/server/...`; bindings are `INSERT INTO /server/...`. E0 ships only the module + mount stub. |
| Engine / Session (threads registries + audit + caps) | §6, §10 | `cfs-core` (`Engine` / `Session`) | The execution context both faces share; the only place the audit sink and capability gate are threaded. |
| Structured, machine-readable errors (AI-facing) | §5 | `CfsError` (in `cfs-core`, re-exported) | One error enum workspace-wide. The AI procedure depends on errors being parseable, not prose. |
| Parser front door | §2.2, §9 | `cfs-parser` (t02) | Turns DSL text → AST sum types. Wired to the library the t02 ADR locks; library types never cross its public boundary. |
...
```
                       ┌──────────────────────────── cfs (bin) ────────────────┐
                       │  main.rs → cfs-cmd::run(args)                          │
                       └───────────────────────────────────────────────────────┘
                                          │ dispatch (one of three arms)
              ┌───────────────────────────┼────────────────────────────┐
        cfs run '<stmt>'            interactive shell              cfs serve <cfg>
         (one-shot)                  (cwd-tagged loop)              (cfs-server)
              └───────────────────────────┼────────────────────────────┘
                                          ▼
                            ┌──────────── cfs-core ────────────┐
                 ...
                 └────────────────┘     │
                         ...
                         (cfs-server is itself a Driver over /server)
```

### Entities

- **Engine** (`cfs-core`) — process-wide context. Owns the three registries, an
...
- **Session** (`cfs-core`) — a single interaction's state: the cwd `{driver, path}` for
...
- **Driver** (trait, `cfs-driver`) — `mount / describe / capabilities / procedures /
  prelude`. The capability surface is what lets unsupported verbs be rejected *at parse
  time* (§5) — the AI-facing structured error.
- **Codec** (trait, `cfs-codec`) — `fmt / decode / encode`, pure `bytes ↔ rows`.
- **Plan / Effect** (`cfs-plan`) — the typed DAG with `irreversible`. In E0 these are
...
|---|---|---|
| `shell.gmailClient` — a *narrow* interface the shell depends on (so commands are fake-able with no creds) | The consumer-side small `Driver` trait (§5, §9 "consumer-side small traits") | `cfs-driver::Driver` must be narrow + fake-able; t01's "dummy in-test impl that performs no I/O" is the direct analog of the Go fake. |
| `internal/gmail` "quarantining the Gmail SDK behind one package" | "SDK/vendor types never leak past a driver boundary (owned DTOs)" (§9) | The `sealed`/marker owned-DTO convention on `cfs-driver`. |
...
| `main.go` dispatch: `auth` / `log` / `__complete` / one-shot / interactive, with a JSON `{"error":…}` envelope on every failure path | CLI faces §7 + structured AI-facing errors §5 | `cfs-cmd` dispatch arms returning a structured `CfsError`; one consistent error contract. |
...
1. **Closed core is faithfully represented** because the frozen keyword set has exactly
   **one home** (`cfs-lang`), test-locked against an RFD §3 golden list. A later ticket
   that wants new behavior cannot add a keyword in its own crate — there is nowhere to put
   it except `cfs-lang`, and the golden test fails if it tries. *Fidelity guard G1: the
...
2. **Open registries are faithfully represented** because extension = `register(...)`
   into one of three registries in `cfs-core`, with no type change to the core. A new
   driver (E4) implements `cfs-driver::Driver` and calls `MountRegistry::register`; it
...
- **G4 — `cfs-server` as a Driver, not a special case.** §8 says the server *is a
  driver*. If E0 stubs `cfs-server::serve` as a bespoke entrypoint that does *not* route
  ...
  `cfs-server` asserting this and a `// TODO(E7): register /server as a Driver` anchor so
  the seam is visibly reserved.

- **G5 — `cfs-cmd` must hold *no* domain logic.** The Go `main.go` already leaks a little
  domain knowledge (path defaults, completion). The RFD's coherence depends on CLI and
  server being *thin dispatch* into shared `cfs-core`. If `cfs-cmd` accretes statement
  handling, the "two faces of one engine" claim degrades into "two engines." Fidelity
  guard: `cfs-cmd` depends on `cfs-core` only; it never depends on `cfs-lang`, `cfs-plan`,
  `cfs-driver`, or `cfs-codec` directly. (`clippy`/a dependency-direction check can
  enforce this.)

- **G6 — `cfs-parser` boundary reversibility.** t02 locks a library (default winnow) but
  the RFD's footprint/reversibility concern (§9) means the chosen library's types must
  not appear in `cfs-parser`'s public API — wrap into an owned `ParseError`. This makes
...
- **B1 — Single binary, dual face (§9).** One `[[bin]] cfs`. CLI and server are dispatch
  arms in `cfs-cmd`, both reaching the *same* `Engine` in `cfs-core`. INTEGRITY: high —
  the split places the shared engine below both faces. RISK: G5 (cmd must stay
  logic-free).

- **B2 — Closed core / open registry (§3).** Frozen keywords in `cfs-lang` (one home);
  open extension via three `register/resolve` registries in `cfs-core`. INTEGRITY: high,
  conditional on G1 (golden test) + G2 (generic over trait objects).

- **B3 — Driver boundary / no vendor leak (§5, §9).** `cfs-driver` exposes a narrow trait
...
- **B4 — Purity / effects-as-data (§3, §6).** Effects exist only as `cfs-plan` data;
...
- **B6 — Parser boundary (§9, t02).** `cfs-parser` owns `parse_statement` + owned
  `ParseError`; library types stay inside. INTEGRITY: high, conditional on G6 (pub-use
  audit). Position: `cfs-parser` depends on `cfs-lang` (for the AST + keyword consts) and
  is consumed by `cfs-core` (which calls `parse_statement` to turn text → AST). It sits
  *between* `cfs-lang` and `cfs-core`'s dispatch, never above `cfs-cmd`.
...
- **B7 — wasm-friendliness (§1, §9, A3).** Core crates avoid threads/`std::fs`/sockets;
  ...
  `cfs-parser` to pass, so the parser crate must honor B7 strictly.)
...
```
cfs (bin) → cfs-cmd → cfs-core → { cfs-lang, cfs-plan, cfs-driver, cfs-codec, cfs-parser }
                          ▲
                    cfs-server ── (is a Driver; depends on cfs-core + cfs-driver)
```

Arrows point toward more-foundational crates; there are **no back-edges** and **no
cycles**. `cfs-cmd` must not reach past `cfs-core`. The leaf crates (`cfs-lang`,
`cfs-plan`, `cfs-driver`, `cfs-codec`) do not depend on each other except where the RFD
requires it (e.g. `cfs-driver` returning a `cfs-plan::Plan` node, `cfs-parser` consuming
`cfs-lang`'s AST). This acyclic spine is what lets later tickets add code inside a single
...
- `cfs` (bin) — entrypoint shell; thin.
- `cfs-cmd` — argv parsing (clap), three dispatch arms (run / shell / serve); logic-free.

**B. Shared engine glue**
- `cfs-core` — `Engine`, `Session`, the three registries, `CfsError`, trait re-exports.
  The hub every face routes through.

**C. Closed-core language surface (frozen)**
- `cfs-lang` — reserved keyword `const` set (golden-locked to RFD §3); AST home (E1).

**D. Effect substrate (effects-as-data)**
- `cfs-plan` — `Effect`, `Plan`, `irreversible`. Anchors the purity invariant.

**E. Open extension seams (the three registries' targets)**
- `cfs-driver` — `Driver` trait + `Archetype` / `Capabilities` / `ProcedureDecl` /
  `AliasFn`; owned-DTO marker convention.
- `cfs-codec` — `Codec` trait (`bytes ↔ rows`).
- (Registries themselves live in `cfs-core` per B/§3.)

**F. Server face (a driver, not a subsystem)**
- `cfs-server` — `serve(config)` stub + `/server` mount stub; reserved seam (G4).

**G. Front door (t02)**
- `cfs-parser` — `parse_statement` + owned `ParseError`; library wrapped (G6); spike
  code quarantined under `cfs-parser/spikes/`; ADR `docs/adr/0001-parser-library.md` is
...
`cfs-parser` is a *new workspace member* that consumes `cfs-lang`'s keyword consts and
produces AST sum types — it cannot exist until the workspace, `cfs-lang`, and the lint/CI
...
- For the Constructor's design and Planner's direction, the one cross-artifact coherence
  ...
  introduces a back-edge (e.g. `cfs-cmd` importing `cfs-lang`, or a leaf crate importing
  `cfs-core`), the "two faces of one engine" claim and the "add inside the seam" property
