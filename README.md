...
**One small grammar for every external service.** `cfs` is a single Rust binary that exposes
...
> [RFD-0001](.workaholic/RFDs/0001-cfs-architecture.md): instead of one FTP-style client for one
> service, cfs is the *closed-core grammar + open registries* control plane that the FTP-shell
> idea generalizes to.

## Why cfs exists (the AI payoff)

cfs exists **for AI**. An agent learns *one* small grammar and *one* operating procedure instead
of N vendor SDKs:

> **DESCRIBE `<path>` → write a cfs statement → PREVIEW → COMMIT**

- **DESCRIBE** a node (`cfs describe /mail/drafts`) returns its archetype, columns, supported
...
operating procedure ships embedded in the binary — run `cfs skill` (and `cfs skill --examples`).
...
- **Least privilege** (RFD §10). Credentials are stored per driver/account (`cfs account add`),
...
sha256 before extracting**, and installs `cfs` to `~/.local/bin` (override with `CFS_INSTALL_DIR`).
...
```sh
cargo build --release    # the native binary at target/release/cfs
```
...
cfs describe /mail/drafts

# 2. write + 3. PREVIEW (default — shows the plan, touches nothing):
cfs run "FROM /mail/inbox |> WHERE subject LIKE '%invoice%' |> SELECT subject, from"

# create a draft (PREVIEW first, then COMMIT to apply):
cfs run "INSERT INTO /mail/drafts VALUES (...)"            # PREVIEW
cfs run "INSERT INTO /mail/drafts VALUES (...) COMMIT"     # apply

# 4. COMMIT an irreversible effect requires an explicit ack in one-shot:
cfs run "id:draft-1 |> CALL mail.send COMMIT" --commit-irreversible
```

The interactive shell (no subcommand) gives the same loop with an FTP-like prompt:

```sh
cfs            # ls / cd / cat / cp / rm … all desugar to the same pipe-SQL plans
```
...
> run locally (set `CFS_DIST_ALLOW=1` only where a clean toolchain + disk exist). The native debug
...
cfs versions follow SemVer, and **the stable public surface is the grammar** (the frozen keyword +
...
([ADR-0005](docs/adr/0005-deployment-hosts.md)). cfs documents the mapping in
