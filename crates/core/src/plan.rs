...
//! `cfs-pushdown` planner using the live [`MountRegistry`], so independent native
//! [`ScanNode`](cfs_pushdown::ScanNode)s surface for T10's batcher.
//!
//! This is the boundary the t14 ticket asks for: `partition_by_source → engine` wired
//! into the engine. `cfs-core` owns the registry (the source of driver pushdown profiles
//! and `describe` schemas), so it is the right home for the adapter that turns the
//! registry into a `cfs_pushdown::SourceRegistry` and lowers a parsed [`Pipeline`] into a
...
use cfs_driver::{Driver, Path, PushdownProfile};
use cfs_parser::{Pipeline, Statement};
use cfs_pushdown::{
...
/// Build a [`cfs_pushdown::SourceRegistry`] from the live [`MountRegistry`]: each mounted
...
pub fn plan_pipeline(
    ...
    let logical = lower_query(pipeline, &source_of, &schema_of)?;
...
mod tests {
    ...
    use cfs_pushdown::explain;
    use cfs_types::{Column, ColumnType};
    use std::sync::Arc;
