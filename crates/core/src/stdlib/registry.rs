...
//! A prelude's alias body is cfs source (`d |> CALL mail.send`). [`StdlibRegistry::
...
use cfs_driver::AliasFn;
use cfs_parser::{parse_statement, PipeOp, Source, Statement};
...
/// One alias a driver declares in its prelude (RFD §5). The `body` is **cfs source** (a
...
pub struct AliasDecl {
    ...
    pub name: String,
    /// The cfs-source body the alias desugars to (e.g. `d |> CALL mail.send`).
    pub body: String,
}

impl AliasDecl {
    /// Construct an alias declaration from a name and a cfs-source body.
    #[must_use]
...
pub struct Prelude {
    ...
    pub driver: DriverId,
    /// The alias declarations (cfs-source bodies).
...
pub enum PreludeError {
    /// An alias body failed to parse as cfs source. Carries the parser error code (the
    /// full [`cfs_parser::ParseError`] is not `Eq`, so we keep the stable code).
    Parse {
        /// The owning driver.
        driver: String,
        /// The alias name.
...
impl StdlibRegistry {
    ...
    ///
    /// # Errors
    /// [`PreludeError::Parse`] if a body is not valid cfs; [`PreludeError::Impure`] if a
...
/// Parse one prelude alias body as cfs source and verify it is a single plan-constructing
...
fn parse_alias_body(driver: &str, name: &str, body: &str) -> Result<String, PreludeError> {
    ...
    // single-CALL alias.
    let calls: Vec<&cfs_parser::CallRef> = pipeline
        .ops
        .iter()
