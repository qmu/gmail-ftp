...
//!
//! A spec is a *fully-parsed* `cfs_parser::Statement` wrapped so it serializes as plain data
...
/// **data** (never a live `cfs_plan::Plan`) so embedding it does not execute it (purity). A
...
const ZERO: cfs_parser::Span = cfs_parser::Span::new(0, 0);
...
pub fn normalize_spans(stmt: &mut Statement) {
    match stmt {
        ...
        Statement::Effect(e) => {
            ...
            match &mut e.body {
                cfs_parser::EffectBody::Values(v) => normalize_values(v),
                cfs_parser::EffectBody::Pipeline(p) => normalize_pipeline(p),
                cfs_parser::EffectBody::SetWhere { set, filter } => {
                    for a in set {
...
fn normalize_values(v: &mut cfs_parser::Values) {
