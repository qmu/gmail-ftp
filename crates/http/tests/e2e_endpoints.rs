...
//! [`cfs_http::serve`] entry and issues genuine HTTP/1.1 requests as raw socket bytes, then
...
//! [`cfs_exec::ReadDriver`]. The federated read endpoint contract is exercised end to end
...
use cfs_server::{Binding, EndpointDef, PolicyDef, ServerState, StatementSource};
use tokio::io::{AsyncReadExt, AsyncWriteExt};
use tokio::net::TcpStream;

use cfs_http::{serve, HttpBinding};
...
/// A live loopback server: spawns the REAL [`cfs_http::serve`] listener on an ephemeral
...
async fn s9_error_bodies_never_leak_a_held_credential() {
    ...
    impl cfs_core::Driver for CredHoldingDriver {
        fn mount(&self) -> &str {
            "/mock"
        }
