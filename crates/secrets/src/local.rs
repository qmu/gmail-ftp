...
//!
//! One encrypted blob at `~/.config/cfs/credentials` (XDG), mode `0600`, holding the whole
...
/// The XDG default path for the credential blob (`$XDG_CONFIG_HOME/cfs/credentials`,
/// falling back to `~/.config/cfs/credentials`). Returns `None` if neither env var is set.
#[must_use]
pub fn default_credentials_path() -> Option<PathBuf> {
    if let Ok(xdg) = std::env::var("XDG_CONFIG_HOME") {
        if !xdg.is_empty() {
            return Some(PathBuf::from(xdg).join("cfs").join("credentials"));
        }
    }
    std::env::var("HOME")
        .ok()
...
mod tests {
    ...
    fn local_store_round_trips_encrypted() {
        ...
        let path = dir.path().join("cfs").join("credentials");
