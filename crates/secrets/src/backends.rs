...
//!   `CFS_SECRET_<DRIVER>_<ACCOUNT>` (upper-cased); listing scans the process env.
//!
...
/// `CFS_SECRET_<DRIVER>_<ACCOUNT>` with both halves upper-cased.
///
...
impl EnvStore {
    /// An `EnvStore` reading the real process environment with the default `CFS_SECRET_`
    /// prefix.
    #[must_use]
    pub fn from_process_env() -> Self {
        Self {
            prefix: "CFS_SECRET_".to_string(),
...
mod tests {
    ...
    fn env_store_reads_fixture_map_and_is_read_only() {
        ...
        vars.insert("CFS_SECRET_MAIL_WORK".to_string(), "env-tok".to_string());
        vars.insert("CFS_SECRET_S3_PROD".to_string(), "s3-tok".to_string());
        let store = EnvStore::from_map("CFS_SECRET_", vars);

        assert_eq!(store.var_name(&key("mail", "work")), "CFS_SECRET_MAIL_WORK");
        assert_eq!(
