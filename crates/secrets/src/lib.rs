//! `cfs-secrets` — the credential / secret store + multi-account resolution (RFD-0001 §10).
//!
//! `cfs` is one binary holding tokens for Gmail, Drive, S3/R2, D1, GitHub, Slack, AWS and
...
//! Owned-DTO only; reuses [`cfs_types::DriverId`] and depends on no other workspace crate,
//! so the spine stays acyclic (`cfs-secrets -> cfs-types`). Resolution ([`resolve`]) and
...
mod tests {
    ...
    fn secrets_is_object_safe_across_backends() {
        let handles: Vec<std::sync::Arc<dyn Secrets>> = vec![
            ...
            std::sync::Arc::new(EnvStore::from_map("CFS_SECRET_", Default::default())),
        ];
        let key = CredentialKey::new(DriverId::new("mail"), AccountId::new("work").unwrap());
        for h in &handles {
