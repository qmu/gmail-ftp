...
pub trait Secrets: Send + Sync {
    ...
    /// (idempotent — `cfs account remove` is replayable).
    ///
    /// # Errors
    /// [`SecretError::Locked`] or [`SecretError::Backend`] on failure.
...
pub enum SecretError {
    /// No credential stored for this `(driver, account)`. Actionable: run `cfs account
...
/// on [`cfs_driver::ProcSig`], passed in as owned labels) the resolved account is granted.
///
