...
/// credentials (RFD §10). Defined here rather than imported from `cfs-driver` so
/// `cfs-types` remains the lowest crate in the spine.
...
/// list ([`Name`]s) rather than the driver `Path` type so `cfs-types` stays a leaf
/// (no dependency on `cfs-driver`); E4 adapts the driver `Path` into segments at the
/// boundary.
