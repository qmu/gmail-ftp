...
//! [`cfs_runtime::SharedApplier`] the runtime's [`cfs_runtime::PlanApplierBridge`] drives
//! under `COMMIT`.
//!
...
//! [`cfs_secrets::Secrets`] handle behind `Arc`s, performing fresh World I/O on every call —
...
impl RestApplier {
    ...
    fn decode(&self, resp: &HttpResponse) -> Result<RowBatch, HttpError> {
        self.codec.decode(&resp.body).map_err(|e| match e {
            cfs_codec::CfsError::Decode { fmt, detail } => HttpError::Decode { fmt, detail },
...
impl PlanApplier for RestApplier {
    /// The introspective `cfs_driver::Driver::applier()` seam (t09): a synchronous,
    ...
    /// `cfs-plan` — and the reason is secret-free by construction.
