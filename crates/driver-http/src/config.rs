...
//! itself. The token is resolved through the injected [`cfs_secrets::Secrets`] handle at
...
use cfs_secrets::{AccountId, CredentialKey, DriverId};
...
/// live token never does. Resolves through [`cfs_secrets::Secrets::get`].
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
