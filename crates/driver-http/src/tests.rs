...
//! credentials: auth secrets come from a `cfs_secrets::InMemoryStore`.

use super::*;
use cfs_codec::JsonCodec;
use cfs_driver::{check_capability, Archetype, Driver, Path, Verb};
use cfs_plan::{Affected, EffectKind, EffectNode, NodeId, Target, VfsPath};
...
fn rest_target(path: &str) -> Target {
    Target::new(cfs_plan::DriverId::new("rest"), VfsPath::new(path))
}
...
fn describe_reports_relational_table_with_open_json_schema() {
    ...
    assert_eq!(d.id(), cfs_plan::DriverId::new("rest"));
...
fn update_and_call_are_terminal_decode_failures() {
    ...
    let call = EffectNode::new(
        NodeId(1),
        EffectKind::Call(cfs_plan::ProcId::new("x.y")),
        rest_target("/rest/example/things"),
    );
