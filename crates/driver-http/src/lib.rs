//! `cfs-driver-http` — the **generic HTTP / REST driver** (RFD-0001 §5, t18): the escape
//! hatch for any service cfs does not model natively. It implements the t13
//! [`cfs_driver::Driver`] contract, mounted at `/rest/<api>/...`, and registers the ad-hoc
...
//! [`HttpClient`] trait, auth injection from a [`cfs_secrets::Secret`], status→error
...
//!   [`cfs_runtime::SharedApplier`] the bridge drives.
//! - [`rest_apply_driver`] — wraps the applier in a [`cfs_runtime::PlanApplierBridge`] ready to
...
//! [`cfs_secrets::Secrets`] handle at commit time; the live token is read via `Secret::expose`
...
use cfs_codec::{Codec, Row, RowBatch, Value};
use cfs_driver::{Archetype, Capabilities, Driver, NodeDesc, Path, ProcSig, PushdownProfile, Verb};
use cfs_plan::{EffectKind, EffectNode, NodeId, PlanApplier, Target, VfsPath};
use cfs_runtime::PlanApplierBridge;
use cfs_secrets::Secrets;
use cfs_types::{Column, ColumnType, DriverId, Schema};
