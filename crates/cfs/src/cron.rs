//! The `cfs serve` **cron composition** (t33): the binary wires the JOB scheduler binding.
//!
//! Like the HTTP binding, `cfs-cron` is a LEAF that consumes `cfs-server` (the registry +
//! reconcile seam) AND `cfs-exec` (the evaluator) — composing it HERE (the terminal binary) keeps
//! `cfs-cmd` off it and lets `cfs-cron`'s feature-gated tokio daemon dead-end in the sink. The
...
//! DO plan via `cfs_exec::build_plan`, and spawns the native daemon loop.
//!
...
/// The committer the binary injects: builds the DO plan via `cfs_exec::build_plan` over the serve
/// engine and applies it. At t33 this is the PREVIEW path (a `RecordingApplier`); the live-driver
/// runtime-interpreter applier is the parked carry-over (see the module doc). Reuses `cfs-cron`'s
/// own `RecordingCommitter` shape — the binary just supplies the serve engine.
pub struct PreviewCommitter {
    inner: cfs_cron::RecordingCommitter,
}
