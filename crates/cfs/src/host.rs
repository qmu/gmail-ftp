//! The `cfs serve` **daemon host** (t36): `TokioHost`, the EC2/Linux [`RuntimeHost`] composed in
//! the terminal binary.
//!
//! This formalizes the EXISTING serve composition behind the [`cfs_host::RuntimeHost`] trait — it
//! does NOT rebuild it. The HTTP listener (`cfs-http`), the cron interval (`cfs-cron`), and the
//! watchtower bus + `/hooks/...` ingest (`cfs-watchtower`) are wired in [`crate::serve`]; the
...
//! [`cfs_host::FileDurableStore`] (watcher cursors / `LAST_RUN` that survive a restart) and the
//! on-disk [`cfs_host::AuditLedger`] (the persistent fired-plan record replacing the in-memory
...
/// its cause-attachment methods formalize the existing `cfs-http`/`cfs-cron`/`cfs-watchtower`
...
impl RuntimeHost for TokioHost {
    ...
    async fn serve_endpoints(&self, set: &BindingSet) -> Result<(), HostError> {
        // The cfs-http listener is wired in crate::serve; record the attached endpoint causes.
        self.ledger
    ...
    async fn schedule_jobs(&self, set: &BindingSet) -> Result<(), HostError> {
        // The cfs-cron interval daemon is wired in crate::serve; record the attached JOB causes.
        self.ledger
    ...
    async fn consume_events(&self, set: &BindingSet) -> Result<(), HostError> {
        // The cfs-watchtower bus + /hooks ingest is wired in crate::serve; record the causes.
