# cfs — AI Operating Procedure

`cfs` exists for you, the AI agent (RFD-0001 §1). Instead of learning N vendor SDKs, you learn
**one** small, filesystem-shaped, pipe-SQL grammar and **one** loop. Every service — mail, drive,
github, slack, sql, git, object storage, and the cfs server itself — is reached the same way.
...
> 2. **Write a cfs statement** — path-as-type, closed-core grammar, against what DESCRIBE told you.
...
```text
cfs describe /mail/drafts -json     # step 1 — the contract
cfs run 'INSERT INTO /mail/drafts …'  # step 2+3 — PREVIEW by default
cfs run 'INSERT INTO /mail/drafts …' --commit   # step 4 — apply
```

`DESCRIBE` is **pure**: no credentials, no I/O, no network. You can always describe a node to
learn how to address it before touching anything. `cfs run` **PREVIEWs by default**; nothing is
applied until you add `--commit` (or a trailing `COMMIT` keyword).

## What DESCRIBE tells you

`cfs describe <path> -json` emits one stable JSON contract (`DescribeReport`). Read these fields:
...
| `pushdown`     | What the source filters/projects natively (`where`/`limit`/…); the rest runs locally in cfs. |
...
1. **Respect the closed core.** The grammar (verbs, keywords) is frozen. You extend cfs **only**
...
1. **DESCRIBE** `cfs describe /mail/drafts -json`:
   ```json
...
1. **DESCRIBE** `cfs describe /drive/my/Reports -json` → `"archetype": "blob_namespace"`,
...
1. **DESCRIBE** `cfs describe /github/acme/web/pulls -json` → `"archetype":
...
1. **DESCRIBE** `cfs describe /sql/pg/orders -json` → `"archetype": "relational_table"`,
...
1. **DESCRIBE** `cfs describe /git/myrepo/commits -json` → git is multi-archetype; the commit log
...
1. **DESCRIBE** `cfs describe /server/triggers -json` → the `/server` self-config driver: a
...
- **PREVIEW is the default.** `cfs run '<stmt>'` previews; add `--commit` to apply.
