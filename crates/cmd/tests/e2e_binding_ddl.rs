...
//! OUTSIDE — either the REAL `cfs serve` binary as a subprocess (boot scenario) or the public
//! `cfs-core` desugar API + the public `cfs-server` `Runtime` as a black box. The stronger
...
//!
//! ## Why no `cfs-parser` here (deliberate constraint, same as the t30 E2E)
//! `tests/dep_direction.rs` forbids `cfs-cmd` from depending on `cfs-parser` (it counts
//! dev-deps). So this test NEVER calls the parser directly. The CREATE forms are driven
//! through `cfs_core::parse_server_binding_ddl` (the core DDL entry point parses internally)
...
//! `cfs_core::{PlanSpec, StatementSpec}::from_canonical` (serde, no re-parse).
//!
...
//!  8. Boot a fixture mixing CREATE forms + INSERT through `cfs serve` and confirm the
...
use cfs_server::{NullBinding, Runtime, ServerError, ServerState};
...
/// Locate the built `cfs` binary (same approach as the t30 E2E harness).
fn cfs_bin() -> PathBuf {
...
fn re_create_of_a_same_named_job_is_upsert_by_name_no_op() {
    // Re-applying the SAME CREATE (the config.cfs replay case) converges — UPSERT-by-name, no
...
fn serve_boots_mixed_fixture_and_drains_audit_on_sigint() {
    // Drive the REAL `cfs serve` binary over the mixed CREATE+INSERT fixture: it boots without
