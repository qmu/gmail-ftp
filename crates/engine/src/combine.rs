...
//! [`PhysicalPlan`](cfs_pushdown::PhysicalPlan), pulling one [`RowBatch`] per native
//! [`Scan`](cfs_pushdown::PhysicalPlan::Scan) from the supplied [`ScanResults`] and
//! folding each local [`Combine`](cfs_pushdown::PhysicalPlan::Combine) op over its inputs.

use cfs_pushdown::{CombineOp, PhysicalPlan};
...
/// stays aligned with [`PhysicalPlan::scans`](cfs_pushdown::PhysicalPlan::scans).
fn binary(
    inputs: &[PhysicalPlan],
    cursor: &mut Cursor,
    op: &'static str,
