...
//! `describe` returns a [`NodeDesc`] = archetype tag + `cfs_types::Schema` (the canonical
...
//! The contract speaks [`Path`]; the effect substrate speaks [`cfs_plan::VfsPath`]. The
...
//! trait signatures need them and the acyclic spine forbids reaching up into `cfs-core`.
//!
...
/// model: the schema is `cfs_types::Schema` (typed columns from t05), and the archetype
...
pub struct NodeDesc {
    ...
    /// The node's typed columns (the canonical `cfs_types::Schema`; powers type-checking).
    pub schema: Schema,
}
...
/// zero-sized marker keeps `cfs_plan::Plan` referenced from the contract crate and
...
mod tests {
    ...
    /// describe returns the canonical typed `cfs_types::Schema` — the reconciliation
    ...
    fn call_builds_irreversible_plan_node_without_io() {
        ...
        let node = EffectNode::new(
            NodeId(0),
            cfs_plan::EffectKind::Call(cfs_plan::ProcId::new("fix.send")),
            target,
        )
