//! The cfs lexer: a pure `&str -> Result<Vec<Spanned<Token>>, LexError>` scanner.
//!
...
//! hand-written byte/char cursor rather than a combinator parser, so `cfs-lang`
...
//!   starts a [`Token::Path`]; cfs has no arithmetic `/` in the core grammar, so
...
/// Tokenize one cfs statement into a flat, spanned token stream.
///
...
///
/// ```
/// use cfs_lang::lex::lex;
/// use cfs_lang::token::Token;
