//! [`GitError`] — the structured, secret-free error taxonomy for the git driver (RFD-0001 §5,
//! AI-consumable). Every variant is owned data the AI can act on; **no** object bytes, ref
//! names beyond the addressed one, or `.git/config` content ever leak here (RFD §10). The local
//! object model needs no credentials, so there is no credential surface to redact.

use thiserror::Error;

/// The git driver's error taxonomy. Maps to [`cfs_driver::CfsError`] at the contract boundary
/// and to a runtime [`cfs_runtime::EffectError`] in the apply leg.
...
pub enum GitError {
    ...
    /// `old` oid (optimistic concurrency, RFD §6). The write is **rejected, never clobbered**.
...
impl GitError {
    /// Map to the shared [`cfs_driver::CfsError`] at the contract boundary (`describe`).
    ...
    /// credential surface).
    #[must_use]
    pub fn into_cfs(self, path: &str) -> cfs_driver::CfsError {
        cfs_driver::CfsError::InvalidPath {
            path: path.to_string(),
            reason: self.code(),
        }
    }
