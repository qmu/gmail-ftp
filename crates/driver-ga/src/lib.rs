//! `cfs-driver-ga` — the **Google Analytics 4 read-only relational `Driver`** (RFD-0001 §5, E4
...
//! **filters**, and GA aggregates server-side and returns rows. That maps onto the cfs
...
//! - [`ga_apply_driver`] — wraps the (reject-all) applier in a [`cfs_runtime::PlanApplierBridge`]
...
//!    `cfs_driver::check_capability` rejects `INSERT`/`UPSERT`/`UPDATE`/`REMOVE` structurally
...
//! is injected by the t19 `GoogleApiClient` and lives behind a [`cfs_secrets::Secret`]; it is
...
//! trait so it mocks in tests (no live GA, no network) and `reqwest` stays in `cfs-driver-http` —
...
impl GaDriver {
    ...
    pub fn run_report(
        ...
        catalog: &Catalog,
    ) -> Result<(Vec<cfs_types::Row>, bool), GaError> {
...
impl Driver for GaDriver {
    ...
    fn describe(&self, path: &Path) -> Result<NodeDesc, cfs_driver::CfsError> {
        ...
        // describable.
        let ga = GaPath::parse(path).map_err(|_| cfs_driver::CfsError::InvalidPath {
        ...
        if ga.is_property() {
            Ok(NodeDesc::new(
                ...
                cfs_types::Schema::new(Vec::new()),
            ))
        } else {
            Err(cfs_driver::CfsError::InvalidPath {
                path: path.as_str().to_string(),
