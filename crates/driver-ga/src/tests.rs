...
use cfs_driver::{check_capability, Archetype, Driver, Path, PushdownProfile, Verb};
use cfs_plan::{EffectKind, EffectNode, NodeId, PlanApplier, Target, VfsPath};
use cfs_types::{CmpOp, ColRef, ColumnType, DriverId, Literal, Pattern, Predicate, Value};
...
fn applier_rejects_every_write_as_read_only() {
    ...
    let shared_err = cfs_runtime::SharedApplier::apply_shared(&applier, &node).unwrap_err();
...
fn errors_never_carry_a_token_or_credential() {
    ...
    let auth: GaError = cfs_google_auth::AuthError::Invalid {
