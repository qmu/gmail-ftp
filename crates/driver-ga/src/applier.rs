...
//! through this applier — the applier seam exists solely because the [`cfs_driver::Driver`]
...
impl PlanApplier for GaApplier {
    /// The introspective `cfs_driver::Driver::applier()` seam (t09). GA is read-only, so this
