//! `cfs-parser` — the parser front door (RFD-0001 §2.2, §3, §9).
//!
...
//! driven over the **t03 token stream** (`cfs_lang::lex` → `Vec<Spanned<Token>>`)
...
pub use cfs_lang::{Keyword, Span, KEYWORDS, OPERATORS};

/// Parse one cfs statement into the owned [`Statement`] AST.
///
/// Lexes the source via `cfs_lang::lex` (t03), then runs the full RFD §3 grammar
