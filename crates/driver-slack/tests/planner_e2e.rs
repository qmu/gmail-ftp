//! Planner-owned **black-box E2E harness** for the t25 Slack driver (cfs-foundation-e0).
//!
...
async fn e2e_commit_post_message_through_interpreter_is_complete() {
    let mock = Arc::new(cfs_driver_slack::MockSlackClient::new());
    let driver = SlackDriver::new(mock.clone() as Arc<dyn SlackClient>);
    let bridge = cfs_driver_slack::slack_apply_driver(&driver);
