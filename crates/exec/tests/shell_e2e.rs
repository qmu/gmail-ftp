...
//! `20260622214650-t28-cli-interactive-shell`), driving the public `cfs_exec::shell` API as the
...
//! single-mount run cannot reach via the real `cfs` binary — **cross-mount `cp`/`mv`** (needs a
...
use cfs_exec::{Completer, ReadDriver, ReadRegistry};
...
fn builtin_cp_matches_typed_upsert_plan() {
    ...
    let previews = |o: &Outcome| -> Vec<cfs_core::Preview> {
        match o {
