...
//! [`ApplyDriver`](cfs_runtime). t20 carry-over closure.
//!
...
//! [`RowBatch`] — exactly the unit [`cfs_engine::ScanResults`] consumes positionally.
//!
//! ## Async, owned, vendor-free (RFD §9)
//! Scans are I/O, so the seam is `async` (object-safe via `async-trait`, like `ApplyDriver`).
//! The driver translates the owned [`PushedQuery`](cfs_pushdown::PushedQuery) inside its own
...
//! the [`MiniEvaluator`](cfs_engine::MiniEvaluator). The seam therefore only promises the
...
use cfs_core::{CfsError, DriverId, RowBatch};
