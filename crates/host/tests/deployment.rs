...
//!
//! These run under the `host-daemon` feature (the conversion from `cfs_server::ServerState` lives
//! there; the wasm-clean core derivation is unit-tested in `src/derive.rs`). No live creds, no
//! network: a fixture `config.cfs` is booted into a `ServerState` through the t30 server `Runtime`
...
use cfs_host::{bindings_from_state, generate_wrangler_toml, NativeStoreKind};
use cfs_server::Runtime;

/// Boot the deployment fixture into a `ServerState` snapshot via the t30 server `Runtime`.
fn boot_fixture() -> cfs_server::ServerState {
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("fixtures")
        .join("deploy_boot.cfs");
...
fn wrangler_toml_matches_the_checked_in_golden() {
    ...
    // names) byte-matches the checked-in golden fixture. Regenerate with CFS_BLESS_GOLDEN=1.
    let state = boot_fixture();
    let set = bindings_from_state(&state);
    let generated = generate_wrangler_toml("cfs-deploy", &set);

    let golden_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("fixtures")
        .join("wrangler.golden.toml");

    if std::env::var("CFS_BLESS_GOLDEN").is_ok() {
    ...
    assert_eq!(
        ...
        "generated wrangler.toml drifted from the checked-in golden \
         (re-bless with CFS_BLESS_GOLDEN=1 if intended)"
    );
