//! Conversion from the live t30 `cfs_server::ServerState` into the owned, host-agnostic
//! [`crate::BindingSet`] (behind `host-daemon`).
//!
//! `cfs-server` pulls tokio `signal` (no-wasm), so this conversion is gated to the EC2/daemon
...
//! `cfs-server`. This is the one place the registry DTOs are projected into the deployment DTOs;
//! both hosts consume the [`crate::BindingSet`] identically afterward.

use cfs_server::{ServerState, StatementSource};
...
/// Boot a `.cfs` config file into a `ServerState` (the in-memory parse→lower→COMMIT path) and
/// derive its [`BindingSet`]. The daemon composition root calls this so the terminal binary never
/// needs a direct `cfs-server` dependency (it stays the thin entrypoint the dep-direction guard
/// pins); the `cfs-server` coupling lives behind `cfs-host`'s `host-daemon` feature.
///
/// # Errors
...
pub fn bindings_from_config(config: &std::path::Path) -> Result<BindingSet, String> {
    let mut rt = cfs_server::Runtime::new();
