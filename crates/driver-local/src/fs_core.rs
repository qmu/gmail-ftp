...
mod verify_tests {
    ...
    fn content_hash_separates_same_length_divergence() {
        let mut a = Fnv1a::new();
        a.update(b"hello-cfs");
        let mut b = Fnv1a::new();
        b.update(b"hexxo-cfs"); // same length, two bytes differ
        assert_eq!(b"hello-cfs".len(), b"hexxo-cfs".len(), "lengths are equal");
        assert_ne!(
            a.finish(),
            b.finish(),
    ...
    fn verify_copy_rejects_length_equal_content_divergent() {
        ...
        std::fs::write(&tmp, b"hexxo-cfs").unwrap();
        let mut src = Fnv1a::new();
        src.update(b"hello-cfs"); // what the source actually was
        let len = b"hello-cfs".len() as u64;
