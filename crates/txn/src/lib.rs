//! `cfs-txn` — the transactional **correctness envelope** (RFD-0001 §6 runtime, §10 safety).
//!
...
//!   `cfs-plan`) is the driver-side dedup point; the ledger covers procs that are not
...
//!
//! ## Spine (acyclic): `cfs-txn → { cfs-plan, cfs-types }`
//! Pure data + orchestration only. `tokio`/`async` stay confined to `cfs-runtime`, which
//! depends on `cfs-txn` and bridges its async `ApplyDriver` to the synchronous [`LegApplier`].
