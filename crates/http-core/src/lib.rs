//! `cfs-http-core` — the shared, **pure leaf** that owns the vendor-free HTTP exchange DTOs and
...
//! Two crates speak owned HTTP DTOs across a synchronous transport seam: `cfs-driver-http` (the
//! generic REST driver, with the real `reqwest` [`HttpClient`]) and `cfs-google-auth` (the OAuth
...
//! `cfs-secrets` (for the one [`cfs_secrets::REDACTED`] marker the manual `Debug` impls emit), so
//! the dep closure is `cfs-http-core → cfs-secrets → cfs-types` — no `reqwest`, no `tokio`, no
//! `cfs-runtime`. The concrete transports stay where they belong: `cfs-driver-http` keeps its
//! `HttpClient` trait + `ReqwestClient`/`MockHttpClient`; `cfs-google-auth` keeps its
