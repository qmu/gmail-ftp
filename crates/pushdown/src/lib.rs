//! `cfs-pushdown` — the **pushdown planner** (RFD-0001 §6 pushdown federation, the
//! federation half of epic E3, ticket t14).
//!
//! A cfs pipeline (`FROM <path> |> <op> |> …`) may straddle several sources. This crate
...
//!    subtree**;
//! 3. negotiates that subtree against the driver's [`PushdownProfile`](cfs_driver::PushdownProfile)
...
//! [`cfs_types::Predicate`] (the typed predicate IR, t05) so a `WHERE` survives into the
