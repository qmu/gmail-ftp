...
//!   source's [`PushdownProfile`](cfs_driver::PushdownProfile) (queried by intent via the
...
//! `cfs-engine` tests assert). Capability/policy denial fails at plan time, never as a
//! partial scan.

use std::collections::BTreeMap;

use cfs_driver::PushdownProfile;
use cfs_types::{Column, ColumnType, Name, Predicate, Schema};
...
/// (each driver's [`Driver::pushdown`](cfs_driver::Driver::pushdown) keyed by its
/// [`Driver::id`](cfs_driver::Driver::id)). Kept here (not a `DriverRegistry` directly)
