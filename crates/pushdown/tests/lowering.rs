//! End-to-end lowering tests: parse real cfs DSL text, lower the AST into a
...
use cfs_types::{Column, ColumnType, Predicate, Schema};
...
fn pipeline_of(src: &str) -> cfs_parser::Pipeline {
...
fn lower_predicate_handles_like_and_in_and_between() {
    ...
    if let cfs_parser::PipeOp::Where(e) = &pipe.ops[0] {
