...
#   * static musl `cfs` daemon binaries for BOTH arches (x86_64 + aarch64-unknown-linux-musl),
#     built with `--features host-daemon` and the size-tuned `[profile.release]`;
#   * the size-optimized `cfs.wasm` Workers module (the `wasm-release` profile: opt-level="z",
#     fat LTO, strip, panic=abort) — the wasm-clean cfs-host core today (the `worker`-backed
...
for target in "${MUSL_TARGETS[@]}"; do
  echo "    target: $target"
  cargo build --release --target "$target" --features host-daemon -p cfs
  cp "target/$target/release/cfs" "$DIST/cfs-$target"
done

echo "==> Building the size-optimized cfs.wasm Workers module (--profile wasm-release)"
# The wasm-clean cfs-host core (the `worker`-backed WorkersHost is parked — ADR-0005). When the
# `worker` crate lands, this builds the `worker` cdylib instead; the artifact name is unchanged.
cargo build --profile wasm-release --target wasm32-unknown-unknown -p cfs-host
WASM_SRC="target/wasm32-unknown-unknown/wasm-release/cfs_host.wasm"
if [[ -f "$WASM_SRC" ]]; then
  cp "$WASM_SRC" "$DIST/cfs.wasm"
else
  # cfs-host is a lib crate; its wasm artifact is the rlib's cdylib when host-workers ships the
  # cdylib crate-type. Until then, record the parked state explicitly rather than faking a binary.
  echo "    NOTE: cfs.wasm cdylib is parked (needs the host-workers cdylib crate-type + worker)." \
    > "$DIST/cfs.wasm.PARKED"
fi
