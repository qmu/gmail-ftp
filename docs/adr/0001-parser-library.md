# ADR 0001 — Parser library for the cfs pipe-SQL DSL
...
**cfs uses [`winnow`](https://crates.io/crates/winnow) (1.0.3) as the DSL parser
...
The library is confined to the crate-private `cfs-parser::grammar` module. The public
surface is the owned `ParseError` (byte span + expected-set + machine code) and owned AST
types; **no winnow type appears in `cfs-parser`'s public API** (fidelity guard G6 / RFD §9
...
| **`\|>`-chain & UPPERCASE ergonomics** | Combinators (`preceded(ws("\|>"), cut_err(pipe_op))`) read directly; UPPERCASE = plain string literals; sourced from the frozen `cfs_lang::Keyword` set. | Equally natural (`just("\|>")`, `choice`, `padded`). | tie |
...
3. cfs statements are short, single-line pipelines, not large source files; multi-error
...
- `cfs-parser` takes a real `winnow` dependency, used **only** inside the private
  `grammar` module. Public API stays owned (`ParseError`, AST). Asserted by
  `cfs-parser::tests::no_vendor_type_in_public_api` and the module being non-`pub`.
- The frozen keyword surface text is sourced from `cfs_lang::Keyword` (RFD §3, one home),
...
error into `ParseError`. **No public signature changes**, so E1+ callers (`cfs-core` →
