...
**Read any label** — the system labels and your own:
...
## Read one message, thread, or attachment

**A single message** by id (or as a file under its label):
...
**A whole thread** (the conversation):

```qfs
id:thread:18f1a2b3c4
```
...
**Trash a whole thread** (the conversation, not one message):

```qfs
remove id:thread:18f1a2b3c4
```
