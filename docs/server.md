...
`qfs serve <config.qfs>` boots the server (RFD §8). A server config is a set of frozen `CREATE …` bindings — sugar over the same write surface the CLI uses. The DDL keywords are part of the closed core (see [language.md](language.md)); a new binding form cannot be invented.
